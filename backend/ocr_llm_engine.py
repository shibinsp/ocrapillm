...
# FastAPI Integration
app = FastAPI(
    title="OCR Database API",
    description="OCR processing with PostgreSQL database integration",
    version="1.0.0"
)
...
@app.get("/")
async def root():
    return {"message": "OCR Database API", "status": "running"}
...
async def upload_and_process_document(file: UploadFile = File(...)):
    """Upload and process a PDF document with database storage"""
    ...
    try:
        ...
        # Process the PDF with database integration
    ...
    try:
        ...
        cursor.execute("""
            SELECT d.id, d.filename, d.original_filename, d.file_size, 
                   d.processing_status as status, d.total_pages as pages,
                   d.upload_date as created_at, d.extraction_method
            FROM documents d 
...
async def get_document_content(doc_id: str):
    ...
    try:
        ...
        cursor.execute("""
            SELECT ec.raw_text, d.total_pages
            FROM extracted_content ec
            JOIN documents d ON ec.document_id = d.id
            WHERE ec.document_id = %s AND ec.page_id IS NULL
